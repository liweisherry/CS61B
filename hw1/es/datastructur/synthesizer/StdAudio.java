 ...
 *  Compilation:  javac es.datastructur.synthesizer.StdAudio.java
 *  Execution:    java es.datastructur.synthesizer.StdAudio
 *  Dependencies: none
 *
...
public final class StdAudio {
    ...
    public static double[] read(String filename) {
        ...
        if (audioFormat.getSampleRate() != SAMPLE_RATE) {
            throw new IllegalArgumentException("es.datastructur.synthesizer.StdAudio.read() currently supports only a sample rate of " + SAMPLE_RATE + " Hz\n"
                    + "audio format: " + audioFormat);
        }

        // require 16-bit audio
        if (audioFormat.getSampleSizeInBits() != BITS_PER_SAMPLE) {
            throw new IllegalArgumentException("es.datastructur.synthesizer.StdAudio.read() currently supports only " + BITS_PER_SAMPLE + "-bit audio\n"
                    + "audio format: " + audioFormat);
        }

        // require little endian
        if (audioFormat.isBigEndian()) {
            throw new IllegalArgumentException("es.datastructur.synthesizer.StdAudio.read() currently supports only audio stored using little endian\n"
                    + "audio format: " + audioFormat);
        }
    ...
    /***************************************************************************
     * Unit tests {@code es.datastructur.synthesizer.StdAudio}.
     ***************************************************************************/
